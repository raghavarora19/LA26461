...
def get(directory, action, port):
    ...
    if request_new[0] == " ":
      ...
      def get(directory, action):
       request = "GET / HTTP/1.0\r\n" + directory + "\r\n\r\n"
       s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
       s.connect(("127.0.0.1", port))
       s.sendall(request.encode())
def post(directory, port):
...
def main():
        ...
        argParser.add_argument("-v", action="store_true", dest="debugging", help="Echo debugging mesages", default=False)
        ...
        if args.re_type == "post" and "POST":
            post(args.directory, args.action, args.port)
main()